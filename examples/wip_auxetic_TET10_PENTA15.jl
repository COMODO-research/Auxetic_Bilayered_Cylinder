...
for (i,f) in enumerate(FE_penta_quad_boundary)
    Xf = Xp[f]
    if all(Xf .< (-axialLength/2.0)+tolLevel)
        push!(indQuadsFront,i)
    elseif all(Xf .> (axialLength/2.0)-tolLevel)
        push!(indQuadsBack,i)
    end
end
...
startStep(file_io; name="Prestretch", nlgeom="YES", type="Static", parameters=[0.1, 1.0, 1e-5, 0.1], inc=1000)
...
startStep(file_io; name="Buckle", nlgeom="NO", type="Buckle", parameters=[1, nothing, 2, 1000], perturbation="")
