...
E_tet,V_tet,CE_tet,Fb_tet,Cb_tet = tetgenmesh(Fb_tet,Vb_tet; facetmarkerlist=Cb_tet, region_vol=vol1, stringOpt)
...
instanceName_1  = "Auxetic_structure"
...
Xp = [v[1] for v in V]
indQuadsFront = Vector{Int}()
indQuadsBack = Vector{Int}()
for (i,f) in enumerate(FE_penta_quad_boundary)
    Xf = Xp[f]
    if all(Xf .< tolLevel)
        push!(indQuadsFront,i)
    elseif all(Xf .> L-tolLevel)
        push!(indQuadsBack,i)
    end
end

indNodesTri = reduce(vcat,Fb_tet[Cb_tet.==2])
indNodesQuad = reduce(vcat,FE_penta_quad_boundary[indQuadsFront])
indFrontNodes = unique([indNodesTri;indNodesQuad])

indNodesTri = reduce(vcat,Fb_tet[Cb_tet.==3])
...
startAssembly(file_io; name="Assembly-1")
    ...
    for (m,i) in enumerate(indFrontNodes)
        ...
        addIndexSet(file_io, nodeSetName_NOW, [i]; type=:nodes, indexOffset=0)
    end
    ...
    for m in 1:length(indFrontNodes)
...
addMaterial(file_io; name=materialName_1, category="Hyperelastic, mooney-rivlin", parameters=[110., 0.1, 0.5])
addMaterial(file_io; name=materialName_2, category="Hyperelastic", parameters=[0.1, 0.4, 0.00200067], user="", type="COMPRESSIBLE", properties="3")

addBoundary(file_io; setName=instanceName_1*"."*nodeSetName_4, flag="ZSYMM")

startStep(file_io; name="Step-1", nlgeom="YES", type="Static", parameters=[0.1, 1.0, 1e-5, 0.1], inc=1000)

addBoundary(file_io; setName=nodeSetName_assembly_REF, vals=[1, 1, 0.12])
...
startStep(file_io; name="Step-2", nlgeom="NO", type="Buckle", parameters=[1, nothing, 2, 1000], perturbation="")
    addBoundary(file_io; setName=instanceName_1*"."*nodeSetName_4, flag="ZSYMM", op="NEW", load_case=1)
    addBoundary(file_io; setName=instanceName_1*"."*nodeSetName_4, flag="ZSYMM", op="NEW", load_case=2)
    
    addBoundary(file_io; setName=nodeSetName_assembly_REF, op="NEW", load_case=1, vals=[1, 1, 0.7])
    addBoundary(file_io; setName=nodeSetName_assembly_REF, vals=[2, 2], skipHeading=true)
    addBoundary(file_io; setName=nodeSetName_assembly_REF, vals=[3, 3], skipHeading=true)

    addBoundary(file_io; setName=nodeSetName_assembly_REF, op="NEW", load_case=2, vals=[1, 1, 0.7])
    addBoundary(file_io; setName=nodeSetName_assembly_REF, vals=[2, 2], skipHeading=true)
    addBoundary(file_io; setName=nodeSetName_assembly_REF, vals=[3, 3], skipHeading=true)
